...
export async function markAttendance(req, res, next) {
  try {
    ...
    if (distance > CAMPUS_RADIUS) {
      await pool.query(
        `INSERT INTO student_geolocation_logs (student_id, latitude, longitude, accuracy, distance, status)
         VALUES (?, ?, ?, ?, ?, 'REJECTED')`,
        [
          student.id,
          latitude,
    ...
    await pool.query(
      `INSERT INTO student_self_markings (student_id, status, marked_at)
       VALUES (?, 'P', NOW())`,
      [student.id]
    );

    await pool.query(
      `INSERT INTO activity_logs (actor_role, actor_id, action, details)
       VALUES ('student', ?, 'SELF_MARK_ATTENDANCE', ?)`,
